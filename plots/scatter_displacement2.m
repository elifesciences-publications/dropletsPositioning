function Fig = scatter_displacement2(DIRS,sigma,lims,plotAvg,sectionLines,append)
...
%     displacement_scatter(i) = scatter([dropVectors(i,:).dropRadius] * 2, blobDistanceMinusPlus(1,:), 30,'filled');
     displacement_scatter(i) = plot([dropVectors(i,:).dropRadius] * 2, blobDistanceMinusPlus(1,:),'.','LineStyle','none','MarkerSize', 18);
    ...
    if plotAvg
        ...
        %     displacement_plot(i).DisplayName = sprintf('%s, N=%d',dropVectors(i,1).description, length(sortedRadius{i}));
    else

...
%     ma = ma.fitLogLogMSD(5);
    ...
    ma.fitMeanMSD(false, [], 10, 'log')
    ma.fitMeanMSD(false, [], 10, 'linear')
    meanMSD = ma.getMeanMSD;
    display(sprintf('D at 1 min: %g +- %g', round(meanMSD(3,2)/6,2,'significant'), round(meanMSD(3,3)/6,1,'significant')));
...
%     ma.plotMeanMSD(ax,false);
    ...
    ma.plotMeanVCorr_smooth(ax3, true);

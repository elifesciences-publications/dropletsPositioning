timepoints = 1:4;
matDIR = {'W:\phkinnerets\storage\analysis\Niv\rambam5 extract\symmetry breaking analysis\nocadazole\2019_10_27 33uM nocadazole mix1 sample1\'};
...
for dirNum = 1:length(allDroplets)
    ...
    for tp=timepoints
        ...
        [FigTransitionRangeNocadazole(dirNum), sectionsNocadazole(tp,:, dirNum)] = scatter_displacement_paper2(dropVectors(dirNum),true,sectionLines, format, outputDir);
    end
...
%     for t=timepoints
%         print(FigTransitionRangeNocadazole(t),fullfile(outputDir,['FigTransitionRangeNocadazole_',num2str(TU(t))]),format);
%     end
% end
% 

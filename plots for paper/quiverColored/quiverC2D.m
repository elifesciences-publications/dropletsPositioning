function [  ] = quiverC2D(x,y,u,v,maxNumArrows,maxVal,scale)
...
Ic = round( I/maxVal*maxColor);
Ic(Ic>maxColor) = maxColor;
Ic( Ic == 0) = 1;
C = colormap;
...
for n = 1:s(1)
    for m = 1:s(2)
        if I(n,m) > 0
            quiver(x(n,m),y(n,m),u(n,m),v(n,m),scale,'Color',C(Ic(n,m),:),'LineWidth',lw,'maxheadsize',hs);
        end
    end
end

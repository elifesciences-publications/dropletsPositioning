function [Fig, ma] = plot_movement_paper3(CCData, name)
    % PLOT_MOVEMENT_XY
...
%     ma = ma.fitLogLogMSD(5);
    ...
    ma.plotRMSD(ax2,[],false);
    ma.fitMeanRMSD(false, [], 10, 'log')
    ma.fitMeanRMSD(false, [], 10, 'linear')
    meanRMSD = ma.getMeanRMSD;
    display(sprintf('D at 1 min: %g +- %g', round(meanRMSD(3,2)/2,2,'significant'), round(meanRMSD(3,3)/2,1,'significant')));
...
%     ma.plotMeanMSD(ax,false);
    ...
    ma.plotMeanVrCorr_smooth(ax3, true);

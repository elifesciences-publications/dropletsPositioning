...
% obj.Border = 'UL';          %'LL'(default), 'LR', 'UL', 'UR', 'NR', 'UN'...
% obj.hTextY_Rot = 0;         %Y-LABEL rotation change as horizontal.
% obj.Calibration = 1;        %length units per pixel calibration
...
classdef scalebar <handle
	...
	% Main properties
		...
		Border='LL'           %'LL', 'LR', 'UL', 'UR', 'NL', 'NR', 'UN', 'LN'
	...
	methods
		function hobj = scalebar(varargin)                        
            ...
            for prop={'XLen','YLen','XUnit','YUnit','hTextY_Rot'...
                      'Position','Border','hTextX_Pos','hTextY_Pos','FontSize','Calibration','Color','LineWidth'}
            ...
            hcmenu = uicontextmenu('parent', hAxes.Parent);
            ...
            hcmenu_text = uicontextmenu('parent', hAxes.Parent);
            ...
            p.addParameter('Calibration', hobj.Calibration);
            p.addParameter('Color', hobj.Color);
            p.addParameter('LineWidth', hobj.LineWidth);
	...
	% Setting properties
		function SetPosition(hobj, varargin)
			...
			set(hobj.hLineY(1), 'XData', XPos*[1 1]);
 			set(hobj.hLineY(2), 'XData', (XPos+hobj.XLen / hobj.Calibration)*[1 1]);
			set(hobj.hLineX(1), 'YData', YPos*[1 1]);
			set(hobj.hLineX(2), 'YData', (YPos+hobj.YLen / hobj.Calibration)*[1 1]);
			set(hobj.hLineY, 'YData', YPos+[0 hobj.YLen / hobj.Calibration]);
			set(hobj.hLineX, 'XData', XPos+[0 hobj.XLen / hobj.Calibration]);
		...
		function SetXLen(hobj, varargin)
            value = hobj.XLen / hobj.Calibration;
		...
		function SetYLen(hobj,  varargin)
            value = hobj.YLen / hobj.Calibration;
        ...
        function SetFontSize(hobj, varargin)
            value = hobj.FontSize;
			if ishandle(hobj.hTextY)
				set(hobj.hTextY, 'FontSize',value);
            end
            if ishandle(hobj.hTextX)
				set(hobj.hTextX, 'FontSize',value);
            end
        end
        function SetCalibration(hobj, varargin)
            %value = hobj.Calibration;
            hobj.SetXLen();
            hobj.SetYLen();
            hobj.SetBorder();
        end
        function SetLineWidth(hobj, varargin)
            value = hobj.LineWidth;
            set(hobj.hLineX, 'LineWidth', value);
            set(hobj.hLineY, 'LineWidth', value);
        end
        function SetColor(hobj, varargin)
            value = hobj.Color;
            if ishandle(hobj.hTextX)
                set(hobj.hTextX, 'Color',value);
            end
   			if ishandle(hobj.hTextY)
                set(hobj.hTextY, 'Color',value);
            end
            set(hobj.hLineX, 'Color',value);
            set(hobj.hLineY, 'Color',value);
        end
	end

function [Fig, ma] = plot_movement_xy5(CCData, name)
    % PLOT_MOVEMENT_XY
    ...
    % Output:
    % Fig       Figure handle.
    subplot = @(M,N,P) subtightplot(M,N,P,[0.06,0.09],[0.19,0.04],[0.02,0.02]);
    S = numel(CCData);
    Fig = figure('Position',[0,0,800,400]);% 'Name', sprintf('Droplets Radius %d - %d um', RLimits), 'NumberTitle','off');
    Fig.Name = name;
    subplot(1,3,1);
...
%     viscircles([0,0],1,'Color','k');
    viscircles([0,0],mean(meanDropRadius*0.75),'Color','k','LineStyle','--');
    mean(meanDropRadius)
    scalebar('XLen',20,'YLen',2, 'XUnit', '{\mu}m', 'Border', 'LN', 'FontSize', 20, 'Color', 'b', 'LineWidth', 2);
...
%     title('Tracks (normalized displacement)');
    ...
    ax.YScale = 'log';
    hold(ax, 'on');
    plot(1:10,(1:10), '--k', 'LineWidth', 2);
    plot(1:10, 20*(1:10).^2, '--k', 'LineWidth', 2);

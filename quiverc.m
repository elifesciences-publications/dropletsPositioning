...
%   QUIVERC(X,Y,U,V,'samesize') makes all the vector the same size. May 18th,2011
%
...
while isstr(varargin{nin}),
  ...
  if ~isempty(vv) & strcmp(lower(vv(1)),'f')
    filled = 1;
    nin = nin-1;
  elseif ~isempty(vv) & strcmp(lower(vv(1:4)),'same')
    samesize=true;
    nin = nin-1;
  else
...
% Define colormap 
vr=sqrt(u.^2+v.^2);
vmax=max(vr(:));vmin=min(vr(:));  %lkuang
...
hold on
 for i=  1:3:imax-1
   ...
   h1= plot(uui(i:i+1),vvi(i:i+1),'linewidth',lw,'color',[c1 c2 c3]);
 end
%--------------------------------------------lkuang
% Add correpsonding colorbar to the plot
%first, get the clim,ytick from hh=colorbar;then convert [vmax,vmin] for yticklabel;
hh=colorbar;
clim=get(hh,'Limits');ytick=get(hh,'ytick');
ytklabel=ytick*(vmax-vmin)/(clim(2)-clim(1))+vmin;
ytklabel=floor(100*ytklabel)/100; %confine it to two digits only for colorbar label
set(hh,'yticklabel',ytklabel)
...
if plotarrows,
 ...
 for i=  1:imax-1
    ...
    if ii==0; ii=1; end   
    c1= CC(ii,1);    c2= CC(ii,2);    c3= CC(ii,3);
    h2=plot(uui(i:i+1),vvi(i:i+1),'linewidth',lw,'color',[c1 c2 c3]);
 end

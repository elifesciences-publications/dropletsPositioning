function CCData = smooth_data_P2(dropX, dropY, dropZ, dropRadius, aggX, aggY, aggZ, aggRadius, calibration, Zcal, timepoints, timeSep, times, order, framelen, vorder, vframelen, magnetOff)
    if numel(framelen) < 2
        framelen(2) = framelen(1);
    end
    CCData.timepoints = timepoints;
...
function smoothedData = doSmooth(data, order, dn, framelen, magnetOff)
    if magnetOff <= framelen(1)
        smoothedData = savitzkyGolayFilt(data, order, dn, framelen(2));
    else
...
function V = doV(data, vorder, vframelen, magnetOff)
    if magnetOff <= vframelen
        V = savitzkyGolayFilt(data, vorder, 1, vframelen);
%         VClean = removeOutliers(V);
    else
        V = [savitzkyGolayFilt(data(1:(magnetOff -1)), vorder,1,vframelen), ...
            savitzkyGolayFilt(data(magnetOff:end), vorder,1,vframelen)];

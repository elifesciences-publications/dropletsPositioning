function DATA = bigdrops_auto_analysis_oop_onefile(PAR)
...
j = 1; %for j=1:length(fs);
    ...
    drops = extract_drops_oop_c(image_stack,PAR,-1,-1,'nodetect');
...
if isfield(DATA.PAR,'NAME')
    out_dir_name = ['analysis_onefile_',DATA.PAR.NAME];
else
    out_dir_name = 'analysis_onefile';
end
full_path = fullfile(DATA.PAR.DIRNAME,out_dir_name);
if ~exist('out_dir_name','dir')
    mkdir(full_path);
end
save(fullfile(full_path,'data_auto_onefile.mat'),'DATA','-v7.3'),
end

function res = bpass(image_array,lnoise,lobjectX,lobjectY,threshold)
% 
% NAME:
%               bpass
% PURPOSE:
...
if nargin < 3, lobjectX = false; end
if nargin < 4, lobjectY = lobjectX; end
...
if lobjectX  
  boxcar_kernelX = normalize(...
      ones(1,length(-round(lobjectX):round(lobjectX))));
end
if lobjectY
  boxcar_kernelY = normalize(...
      ones(1,length(-round(lobjectY):round(lobjectY))));
end
...
if lobjectX
  bconv = conv2(image_array',boxcar_kernelX','same');
else
  bconv = image_array';
end
if lobjectY
  bconv = conv2(bconv',boxcar_kernelY','same');
else
  bconv = bconv';
end

if lobjectX || lobjectY
...
lzeroX = max(lobjectX,ceil(5*lnoise));
lzeroY = max(lobjectY,ceil(5*lnoise));

filtered(1:(round(lzeroY)),:) = 0;
filtered((end - lzeroY + 1):end,:) = 0;
filtered(:,1:(round(lzeroX))) = 0;
filtered(:,(end - lzeroX + 1):end) = 0;
